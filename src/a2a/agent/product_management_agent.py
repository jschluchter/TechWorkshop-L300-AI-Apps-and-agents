...
def _normalize_gpt_endpoint(endpoint: str) -> str:
    """Normalize endpoint to the Azure resource base URL.

    Handles values that already include API paths, such as:
    - /openai/v1/responses
...
def get_chat_client() -> OpenAIChatClient:
    ...
    normalized_endpoint = _normalize_gpt_endpoint(endpoint)
...
class AgentFrameworkProductManagementAgent:
    ...
    async def invoke(self, user_input: str, session_id: str) -> dict[str, Any]:
        ...
        # Try strict structured output first; gracefully degrade if model returns malformed JSON.
        try:
            ...
            error_text = str(exc)
            if "ResponseFormat" in error_text or "json_invalid" in error_text or "Invalid JSON" in error_text:
                logger.warning(
                    "Structured output validation failed; retrying without strict response_format",
                    exc_info=True,
                )
                retry_response = await self.agent.run(
                    messages=user_input,
                    session=self.session,
                )
                return self._get_agent_response(retry_response.text)
            raise
    ...
    def _get_agent_response(
        ...
        default_response = {
            ...
            'content': message_text or 'No response available',
        }
        ...
        try:
            ...
            logger.info('Message did not come in valid ResponseFormat JSON; using fallback parser.')
            # If it looks like JSON, try a lenient read before falling back to raw text.
            try:
                payload = json.loads(message_text)
                status = payload.get('status', 'completed')
                content = payload.get('message', message_text)
                status_map = {
                ...
                mapped = status_map.get(status, status_map['completed'])
                return {**mapped, 'content': str(content)}
            except Exception:
                pass
        except Exception:
            logger.error('An unexpected error occurred while processing the message.', exc_info=True)
            default_response = {
